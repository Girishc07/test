...
#pip3 install --upgrade --force-reinstall tensorflow-gpu==1.15.5
...
def getAudioSetFeatures(fname):
    ...
    #pproc = vggish_postprocess.Postprocessor('//content/drive/MyDrive/final_audio/tagger/vggish_pca_params.npz')
    ...
    with tf.Graph().as_default(), tf.Session() as sess:
        ...
        #vggish_slim.load_vggish_slim_checkpoint(sess, '//content/drive/MyDrive/final_audio/tagger/vggish_model.ckpt')
...
def giveThres(pred, thres=0.015, showSpeechMusic=False):
    ...
    for index in sort:
        Dict1={}
        classes2displaynames[index]=classes2displaynames[index][0:len(classes2displaynames[index])]
        Dict1['class']=str(classes2displaynames[index])
        Dict1['confidence']=round(pred[index]/predSum*100,2)
        l.append(Dict1)
...
#################################################################
def subWrite(f, num, sub, Dict2):
    Dict3={}
    f.write(str(num+1))
    f.write("\n")
    (start, end) = getStartEnd(num)
    f.write(start)
    Dict3['start_time_offset']=num
    f.write(' --> ')
    Dict3['end_time_offset']=num+1
    f.write(end)
    Dict3['classifications']=sub[1]
    f.write("\n")
    f.write(sub[0])
    s="audio_tagger_annotations"+str(num)
    Dict2[s]=Dict3
    f.write("\n")
    f.write("\n")
    return Dict2
...
def genSubsThres(preds, fname, thres, ifShowMusicSpeech=False):
    ...
    with open(srtName, "w") as f:
        for i in range(len(preds)):
            ...
            if(i==len(preds)-1):
              Dict4=subWrite(f, i, sub, Dict2)
              return Dict4   
            subWrite(f, i, sub, Dict2)       
#################################################################
...
if __name__ == '__main__':
    ...
    Dict2={}
    Dict2=genSubsThres(preds, FILE_NAME, THRESHOLD, SHOW_MUSIC_SPEECH)
    giveSumRes(pred, THRESHOLD, SHOW_MUSIC_SPEECH)
    dict={}
    dict['audio_annotations']=Dict2
    j = json.dumps(dict)
